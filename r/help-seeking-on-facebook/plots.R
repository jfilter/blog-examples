...
draw_chart <- function(df, prefix_column, x_title, new_levels, indep_col_name, question, titles) {
  for (i in 1:length(titles)) {
    ...
    dir.create(dir_path, recursive=TRUE, showWarnings=FALSE) # warning if folder already existent
    file_name <- paste(x_title, titles[i], ".png", sep="")
...
draw_chart(data, "f_", "Frequency", f_levels, "gender", f_question, titles)
draw_chart(data, "e_", "Effectivity",  e_levels, "gender", e_question, titles)
...
draw_chart(data, "f_", "Frequency", f_levels, "age", f_question, titles)
draw_chart(data, "e_", "Effectivity",  e_levels, "age", e_question, titles)
